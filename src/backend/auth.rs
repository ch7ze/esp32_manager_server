...
mod tests {
    ...
    fn test_valid_permission() {
        assert!(is_valid_permission("R"));
        assert!(is_valid_permission("W"));
        assert!(is_valid_permission("V"));
        assert!(is_valid_permission("M"));
        assert!(is_valid_permission("O"));
        assert!(!is_valid_permission("X"));
    ...
    fn test_device_creation() {
        ...
        assert_eq!(device.owner_id, "user123");
        assert!(device.permissions.contains_key("user123"));
        assert_eq!(device.permissions.get("user123"), Some(&"O".to_string()));
    }
}