...
export class Triangle extends AbstractShape implements Shape {
    ...
    isEqual(shape: Shape): boolean {
        ...
        const otherTriangle = shape as Triangle;
    ...
    private calculateTriangleArea(p1: Point2D, p2: Point2D, p3: Point2D): number {
        return Math.abs((p1.x * (p2.y - p3.y) + p2.x * (p3.y - p1.y) + p3.x * (p1.y - p2.y)) / 2);
    }
    
    clone(): Shape {
        const clonedTriangle = new Triangle(
            new Point2D(this.p1.x, this.p1.y),
            new Point2D(this.p2.x, this.p2.y),
            new Point2D(this.p3.x, this.p3.y)
        );
...
export class TriangleFactory extends AbstractFactory<Triangle> implements ShapeFactory {
    public label: string = "Dreieck";
    private pointCount: number = 0;
    private secondPoint: Point2D;

    constructor(readonly shapeManager: ShapeManager) {
        super(shapeManager);
    }
    
    createShape(from: Point2D, to: Point2D, third?: Point2D): Triangle {
        if (third) {
    ...
    handleMouseDown(x: number, y: number) {
        if (this.pointCount === 0) {
            ...
            if ((window as any).colorState) {
                console.log('TriangleFactory: Applying current colors to new triangle', (triangle as any).id);
                (window as any).colorState.applyColorsToShape(triangle);
            } else {
    ...
    handleMouseMove(x: number, y: number) {
        ...
        if (this.pointCount === 1) {
            ...
            (tmpTriangle as any).id = tempId;
            this.tempShapeId = tempId;
            
            // Apply current colors to temporary triangle for preview
            if ((window as any).colorState) {
                (window as any).colorState.applyColorsToShape(tmpTriangle);
            }
