...
function setupEventHandlers() {
    ...
    console.log('EVENT-WRAPPER DEBUG: setupEventHandlers called from:', new Error().stack?.split('\n')[1]?.trim());
    
    if (!window.canvas || !window.eventBus) {
        console.warn('Canvas or event system not ready yet, retrying in 500ms...');
        console.warn('EVENT-WRAPPER DEBUG: Missing:', 
    ...
    if (event.isReplay) {
        ...
        try {
            ...
            if (shape) {
                ...
                console.log('Available global shape classes:',
                    'Point2D:', !!window.Point2D,
                    'Line:', !!window.Line,
                    'Circle:', !!window.Circle,
                    'Rectangle:', !!window.Rectangle,
                    'Triangle:', !!window.Triangle
                );
                console.log('Available drawer namespace:',
...
function handleShapeDeleted(event) {
    if (event.isReplay) {
        ...
        console.log('DEBUG handleShapeDeleted: window.canvas._originalRemoveShapeWithId exists:', !!window.canvas?._originalRemoveShapeWithId);
        console.log('DEBUG handleShapeDeleted: window.canvas.removeShapeWithId exists:', !!window.canvas?.removeShapeWithId);
        if (window.canvas) {
...
function patchCanvasMethods() {
        ...
        if (!isTemp) {
            ...
            if ((shape as any).from && (shape as any).to && !(shape as any).p1) {
                if (typeof (shape as any).radius === 'undefined') {
                    data.from = { x: (shape as any).from.x, y: (shape as any).from.y };
                    data.to = { x: (shape as any).to.x, y: (shape as any).to.y };
                }
            } else if ((shape as any).center && typeof (shape as any).radius === 'number') {
                data.center = { x: (shape as any).center.x, y: (shape as any).center.y };
                data.radius = (shape as any).radius;
            } else if ((shape as any).p1 && (shape as any).p2 && (shape as any).p3) {
                data.p1 = { x: (shape as any).p1.x, y: (shape as any).p1.y };
                data.p2 = { x: (shape as any).p2.x, y: (shape as any).p2.y };
                data.p3 = { x: (shape as any).p3.x, y: (shape as any).p3.y };
            }
            ...
            if (window.eventBus && !window._isReplaying && !(window as any)._isDragging) {
    ...
    window.canvas.removeShape = function(shape, redraw = true) {
        ...
        if (shape && shape.id && !this.tempShapes[shape.id]) {
            ...
            const isDraggingButAllowDelete = (window as any)._isDragging && eventData.type === 'SHAPE_DELETED';
            if (window.eventBus && (!(window as any)._isDragging || isDraggingButAllowDelete)) {
                window.eventBus.publish(eventData);
            }
        }
...
function patchShapeModificationMethods() {
    ...
    if (window.AbstractShape) {
        ...
        window.AbstractShape.prototype.setFillColor = function(color) {
            ...
            (window.AbstractShape.prototype._originalSetFillColor as any).call(this, color);
        ...
        window.AbstractShape.prototype.setStrokeColor = function(color) {
            ...
            // Call original method
            (window.AbstractShape.prototype._originalSetStrokeColor as any).call(this, color);
        ...
        window.AbstractShape.prototype.setZIndex = function(zIndex) {
            ...
            // Call original method
            (window.AbstractShape.prototype._originalSetZIndex as any).call(this, zIndex);
...
function handleShapeSelected(event: any): void {
    if (!event.isFromServer) return; // Only handle remote selections
    ...
    if (!(window as any)._remoteSelections) {
        (window as any)._remoteSelections = new Map();
    }
    ...
    (window as any)._remoteSelections.set(shapeIdNum, {
        clientId: event.clientId,
        userColor: event.userColor
    });
    
    console.log('EVENT-WRAPPER: _remoteSelections after add:', (window as any)._remoteSelections);
    
    // Trigger canvas redraw to show remote selection
    if ((window as any).canvas && (window as any).canvas.draw) {
        (window as any).canvas.draw();
    }
}

// Handle shape unselection events  
function handleShapeUnselected(event: any): void {
    if (!event.isFromServer) return; // Only handle remote unselections
    ...
    if ((window as any)._remoteSelections) {
        ...
        (window as any)._remoteSelections.delete(shapeIdNum);
        console.log('EVENT-WRAPPER: _remoteSelections after remove:', (window as any)._remoteSelections);
    }
