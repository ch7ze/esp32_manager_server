...
export function drawSelectionHandle(ctx, x, y, color = window.currentUserColor || "#0066cc") {
    const handleSize = 6;
    ctx.save();
    ctx.fillStyle = color;
    ...
    try {
        ...
        switch (shapeData.type) {
            case 'Line':
                ...
                shape = new Line(new Point2D(shapeData.from.x, shapeData.from.y), new Point2D(shapeData.to.x, shapeData.to.y));
                break;
            case 'Rectangle':
                if (!validatePoints(shapeData.from, shapeData.to)) {
                    throw new Error("Invalid Rectangle data: missing or invalid points");
                }
                shape = new Rectangle(new Point2D(shapeData.from.x, shapeData.from.y), new Point2D(shapeData.to.x, shapeData.to.y));
                break;
            case 'Circle':
                ...
                shape = new Circle(new Point2D(shapeData.center.x, shapeData.center.y), shapeData.radius);
                break;
            case 'Triangle':
                if (!validatePoints(shapeData.p1, shapeData.p2, shapeData.p3)) {
                    throw new Error("Invalid Triangle data: missing or invalid points");
                }
                shape = new Triangle(new Point2D(shapeData.p1.x, shapeData.p1.y), new Point2D(shapeData.p2.x, shapeData.p2.y), new Point2D(shapeData.p3.x, shapeData.p3.y));
                break;
            default:
...
function isValidColor(color) {
    ...
    if (predefinedColors.indexOf(color) >= 0)
        return true;
